...
export default function Sidebar() {
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <aside className="hidden md:flex fixed left-0 top-0 h-full w-[220px] bg-sidebar flex-col z-40">
        <div className="px-6 py-6 border-b border-sidebar-fg/10">
          <Link href="/dashboard" className="block">
            ...
            <span className="block text-sidebar-fg/40 text-xs mt-0.5 font-body">
              Objetivos 2026
            </span>
          </Link>
        </div>
          ...
          {navItems.map((item) => (
            <Link
              ...
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-body transition-colors ${
                isActive(item.href)
                  ? 'bg-sky/15 text-sky'
                  : 'text-sidebar-fg/60 hover:bg-sidebar-fg/10 hover:text-sidebar-fg'
              }`}
            >
        ...
        <div className="px-4 py-4 border-t border-sidebar-fg/10 flex items-center justify-between">
          <p className="text-sidebar-fg/30 text-xs font-body">Ignacio — 2026</p>
          <ThemeToggle />
        </div>
      </aside>

      {/* Mobile bottom nav */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-sidebar border-t border-sidebar-fg/10 flex z-40" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
        {[navItems[0], navItems[4], navItems[5], navItems[6], navItems[7]].map((item) => (
          <Link
            ...
            className={`flex-1 flex flex-col items-center gap-1 py-4 text-xs font-body transition-colors ${
              isActive(item.href) ? 'text-sky' : 'text-sidebar-fg/50'
            }`}
          >
